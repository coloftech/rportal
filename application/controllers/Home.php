...
class Home extends CI_Controller
{
	...
	{
		$data['title'] = 'Research Hub';
		$data['content'] = '<h2>Welcome to BISU BILAR Resource Portal</h2>
...
A Premier Science and Technology University for the formation of a world class and Virtues human resource for sustenable development in Bohol and the Country.';
		$data['username'] = $this->session->username;
		$data['visits'] = $this->pagecounter->visit_total($this->pagecounter->get_pageUrl());
		$this->load->view('common/header',$data);
		$this->load->view('home/home',$data);
...
<label>Goal</label><br />
1. Pursue faculty and education excellence and strengthen the current viable curricular programs and develop curricular programs that are responsive to the demands of the times both in the industry and the environment.<br />
2. Promote quality research outputs that respond to the needs of the local and national communities.<br />
3. Develop Communities through Responsive Extension Programs.<br />
4. Adopt Efficient and Profitable Income Generating Projects/Enterprise for Self sustainability.<br />
5. Provide adequate, self-of-the-art and accessible infrastructure support facilities for quality education.<br />
6. Promote efficient and effective good governance supportive of high quality education.';
	...
	public function search($q='',$next_page=false)
	{
		# code...
		if($this->input->get('q')){
		...
		if($q != '-q-'){
				...
				if($this->aauth->is_loggedin()){
					...
					$data['links'] = $this->paging($q,$search,$total,$limit,$start);
		...
		$data['title'] = 'Public user';
		$this->load->view('common/header',$data);
		$this->load->view('public/index.php',$data);
		$this->load->view('common/footer',$data);
	}
	public function paging($q='',$search='',$total=0,$limit=0,$start=0)
	{
	...
	public function read($id=0)
	{
		# code...
        	...
        	//$researcher = $this->post_m->list_researchers($id);
		...
		$this->load->view('common/header',$data);
		$this->load->view('user/readinfo.php',$data);
		$this->load->view('common/footer',$data);
	...
	public function viewer($value='')
	{
		...
		$this->load->view('common/header',$data);
		$this->load->view('user/viewer',$data);
		$this->load->view('common/footer',$data);
	}
